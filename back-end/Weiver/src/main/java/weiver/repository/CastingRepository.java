...
public interface CastingRepository extends JpaRepository<Casting, Long> {
    ...
    @Query("SELECT new weiver.dto.ResponseCastingDTO(a.id, c.role, a.name, a.profileImage) FROM Casting c JOIN Actor a ON c.actorId.id = a.id WHERE c.musicalId.id = :id")
