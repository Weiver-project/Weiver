...
public class ActorService {
	...
	public void saveData(List<String> musicalIds) {
		Document doc = null;
		...
		for(String musicalId : musicalIds) {			
			...
			try {
				...
				for(Element e : actorRoleList) {
					...
					Elements actorImgHtmls = e.select("img");
					
					// 해당 Role에 맞는 배우 ID를 actorIds에 저장한다.
					for(int i = 0; i < actorImgHtmls.size(); i++) {
						String actorId = actorImgHtmls.get(i).attr("src").split("_")[1];
	...
	private void saveActor(String actorId) {
		...
		if(actorRepository.findById(actorId).isEmpty()) {
			...
			try {
				...
				// 배우 이름
				String actorName = doc.select("div.psdetail_subject > table > tbody > tr > td > span.title").text().split("\\|")[0].trim();
				
				if(actorImage != null && actorName != null) {
...
//			System.out.println("이미 존재하는 " + actorId + "입니다.");
		}
	}
...
//			System.out.println("이미 존재하는 " + actorId + ", " + musicalId + ", " + role + "입니다.");
		}
	}
